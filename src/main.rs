...
fn main() {
	...
	registry.push_callback("click_da_buttn", callbacks::Click::from(click_da_buttn)).unwrap();
	registry.push_callback("click_da_other_buttn", callbacks::Click::from(|button: &mut UiButton| {println!("{} bein super clickd", button.label());})).unwrap();
